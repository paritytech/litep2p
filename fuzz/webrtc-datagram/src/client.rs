...
//!
//! This is deliberately not a full libp2p client: it does not run the Noise handshake, so it never
//! authenticates. It only needs to get channel 0 open so the fuzzer's bytes reach the framing and
//! decode path behind DTLS.
...
fn build_client_rtc(local: SocketAddr, remote: SocketAddr) -> (Rtc, ChannelId) {
    ...
    // client identity does not matter here because Option 1 never runs the Noise handshake.
