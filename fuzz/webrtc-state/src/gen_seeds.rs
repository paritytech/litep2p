...
fn connection_seeds() -> Vec<(&'static str, ConnectionScript)> {
    ...
    vec![
        ...
        (
            ...
            // every byte here is appended to a buffer that can no longer be parsed. The
            // first follow-up op alone carries the buffer past the frame cap; two make the
            // shape obvious to whoever reads the seed.
            //
