...
Five [`ziggy`](https://github.com/srlabs/ziggy) harnesses. None is reachable from the repo
root: the root `Cargo.toml` has no `[workspace]` table, so `cargo build` and `cargo test` at
the root do not touch them. The three WebRTC harnesses share the workspace in `fuzz/Cargo.toml`,
and so one `Cargo.lock` and one target directory; `simple` and `structure-aware` are excluded
from it and keep their own.
...
Every harness has ordinary `cargo test` self-checks. Run them first. A harness whose own
assertions are subtly wrong reports itself as a finding on the first run, and one whose seeds
do not decode reports execs while doing nothing:

```sh
cd fuzz
cargo test               # all three WebRTC harnesses
```

Then fuzz one target at a time. The target name is required, not decoration: each crate also
declares a `gen_seeds` binary, so an omitted name is ambiguous.
...
Substitute `webrtc-state` or `webrtc-datagram` for the other two. `-i` is the *initial* corpus
and is only read, so the committed seeds are never mutated in place.
...
`webrtc-datagram` ships a seed corpus under `corpus/` too. Every seed begins with the
target-selector byte. The `preauth-*` seeds are noise-channel `unsigned-varint length ++ body`
frames (a valid frame, a split frame, and the permanent framing errors); the `substream-*` seeds are
multistream-select lines wrapped in a `WebRtcMessage`, for the post-auth substream path.

The server's node identity is a fixed key, so its peer id is stable across runs, but its DTLS
certificate is generated once per process rather than committed. Nothing reads the certhash: the
client turns fingerprint verification off and picks up the server's real fingerprint at runtime.

Keep seeds small. AFL throughput scales inversely with input size, so a fat seed slows every
mutation derived from it; size a seed to the boundary it is meant to cross and no further.
...
- **`webrtc-state` starts from a known finding.** The `conn-wedged-buffer-growth` seed
  reproduces an open defect: `extract_framed_message` does not consume on error, and
  `on_inbound_data` neither drops the reassembly buffer nor closes the channel, so a
  permanently unparseable frame stays at the head and every later byte the peer sends is
  appended to a buffer that can never be parsed — unbounded, remotely triggered memory
  growth on an `Open` channel, which has no opening deadline to bound it. Expect a crash
  within the first iterations of a fresh campaign. `KNOWN_FAILING_SEEDS` in `src/main.rs`
  keeps `cargo test` honest about it; when the defect is fixed, that list empties and
  `permanent_parse_error_wedges_buffer` inverts.
